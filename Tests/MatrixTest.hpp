...
TEST(MatrixTest, matrix_serialization){
    ...
    EXPECT_TRUE(data == a.getArray()); //test serialization
...
TEST(MatrixTest, matrix_add){
    ...
    EXPECT_TRUE(expected == a+b); //add two matrices
    //test subtraction
    expected.fillArray({0,0.4,0.1,0});
    EXPECT_TRUE(expected == (a-b)); //subtract two matrices
}
...
TEST(MatrixTest, matrix_utils){
    ...
    EXPECT_TRUE(from_vec == not_from_vec);
    //matrix to vector
    EXPECT_TRUE(vec == not_from_vec );
    //matrix identity
    Matrix identity(2,2);
    identity.setIdentity();
    Matrix expected_identity(2,2);
    expected_identity.fillArray({1,0,0,1});
    EXPECT_TRUE(identity == expected_identity );
    //square matrix
    Matrix of_size(2);
...
TEST(MatrixTest, matrix_mul){
    ...
    EXPECT_TRUE( vector_times_e == from_vector * e);
}
